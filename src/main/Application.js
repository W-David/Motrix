...
import {APP_RUN_MODE, AUTO_CHECK_UPDATE_INTERVAL, AUTO_SYNC_TRACKER_INTERVAL, PROXY_SCOPES} from '@shared/constants'
...
export default class Application extends EventEmitter {
	...
	watchProxyChange() {
		...
		this.configListeners[key] = userConfig.onDidChange(key, async (newValue, oldValue) => {
			...
			const system = enable && server && scope.includes(PROXY_SCOPES.DOWNLOAD) ? {'all-proxy': server, 'no-proxy': bypass} : {}
	...
	stop() {
		try {
			const promises = [this.stopEngine(), this.shutdownUPnPManager(), this.energyManager.stopPowerSaveBlocker(), this.trayManager.destroy()]
